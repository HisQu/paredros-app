    ...
    return (
        ...
        >
            ...
            <Panel position="top-left">
                ...
                <div
                    className="mb-5 p-3 bg-slate-100 rounded-md border border-slate-200 max-h-60 overflow-y-auto shadow-sm">
                ...
                <div className="mb-5 p-3 bg-slate-100 rounded-md border border-slate-200 shadow-sm">
                    {next_parse_step_info?.step_type === "Token consume" ? (
                        <div className="flex items-center gap-2">
                            ...
                            <code
                                className="px-2 py-1 bg-violet-100 text-violet-800 rounded-md text-sm font-mono">{next_parse_step_info.current_token_repr}</code>
                        </div>
                    ) : (
                        <>
                                ...
                                {next_parse_step_info?.possible_transitions && next_parse_step_info.possible_transitions.length > 0 ? (
                                    <ul className="divide-y divide-slate-300">
                                        {next_parse_step_info.possible_transitions.map((t, i) => {
                                            ...
                                            return (
                                                <li key={i}
                                                    className={`py-1.5 px-2 rounded-md ${isChosen ? "bg-blue-100" : ""}`}>
                                                    <code
                                                        className={`text-sm flex items-center gap-2 ${isChosen ? "text-blue-700 font-bold" : "text-gray-800"}`}>
                                                        {isChosen && <span className="text-lg">&#8594;</span>}
                                                        <span>{t.matches}</span>
                                                        {isChosen && <span
                                                            className="font-medium text-xs bg-blue-200 text-blue-900 px-1.5 py-0.5 rounded-full">(chosen)</span>}
                                                    </code>
                                                </li>
                                            );
                                        })}
